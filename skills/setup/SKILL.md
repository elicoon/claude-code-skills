---
name: setup
description: First-run onboarding for Personal OS / dev-org skills - configure paths, integrations, and preferences
---

# /dev-org:setup

**Purpose:** Conduct first-run onboarding for users who have installed the dev-org skills but haven't configured them yet. Creates a `.dev-org.yaml` config file and optional folder structure.

## When to Use

Use this skill when:
- First installing the dev-org skills from a plugin marketplace or public repo
...
Look for an existing `.dev-org.yaml` in the current working directory.

**If config exists:**
> I found an existing `.dev-org.yaml` configuration.
>
> **Current settings:**
> - Name: [name]
> - Backlog: [path]
...
>
> This will create a `.dev-org.yaml` config file and optionally set up folder structure for your tasks and learnings.
...
Generate `.dev-org.yaml` with gathered information:

```yaml
# Dev-Org Personal OS Configuration
# Generated by /dev-org:setup on [YYYY-MM-DD]
version: 1
...
After creating `.dev-org.yaml`, ensure all configured paths exist:
...
[To be filled in through /dev-org:review]
...
[New lessons captured via /dev-org:review will be added here]
```
...
[New memories captured via /dev-org:review will be added here]
```
...
1. Read back `.dev-org.yaml` and display it
2. List created folders
3. Confirm everything is in place

> **Setup Complete!**
>
> **Configuration:** `.dev-org.yaml`
> ```yaml
...
>
> **Next steps:**
> - Use `/dev-org:add` to capture your first task
> - Use `/dev-org:orient` to review your backlog and plan your day
> - Use `/dev-org:review` to capture learnings after work sessions
...
| **Config file write fails** | "I couldn't write `.dev-org.yaml`. Check file permissions and try again." |
| **Existing config is malformed** | "The existing `.dev-org.yaml` has syntax errors. Would you like me to create a fresh config?" |
| **User wants to abort** | "No problem. Run `/dev-org:setup` when you're ready to configure." |

---

## Files Read

- `.dev-org.yaml` (if exists) - Existing configuration
- `backlog/tasks/` (if exists) - Check for existing backlog
- `reference/` (if exists) - Check for existing reference layer

## Files Written

- `.dev-org.yaml` - Main configuration file
...
>
> **Next steps:**
> - `/dev-org:add` - Capture your first task
> - `/dev-org:orient` - Plan your day
